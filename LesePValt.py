...
urlPV1 = "http://192.168.178.49"
urlPV2 ="http://192.168.178.53"
url1 = "http://192.168.178.49/status.html"          #Anlage1
url2 ="http://192.168.178.53:8050/getOutputData"    #Anlage2

def Login():
    print("erste Anfrage")
    r = requests.post(urlPV1)
    
    if r.status_code == 401:    
        response = requests.get(urlPV1, auth=HTTPBasicAuth('admin', 'admin'))
        print ("Erfolg")
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ 
def ServerAnpingen(url):
    ...
    if response == 0:
        print("Server " + url + " ist erreichbar.")
    else:
        print("Server " + url + " ist nicht erreichbar.")
