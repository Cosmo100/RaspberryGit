...
#Wasseruhr mit API-URL abfragen,
#Json Paket zerlegen 
#Überprüfen, ob Wert gültig ist (no error)
#wenn ja -> Speichern als Text in Datei in AI_onTheEdge_Wasser.heis
...
import os
filename ="/mnt/ramdisk/AI_onTheEdge_Wasser.heis"

APIURL = "http://192.168.178.37/json"
######################### Subroutinen ############################
def WasserdatenZuHeisopo(Zeitpunkt, Zaehlerstand):
...
def JsonTextZerlegen(Daten):
	...
	if (Fehler == "no error"):
		...
		with open(filename, "w",) as file:
			...
			WasserdatenZuHeisopo(Zeitpunkt, Zaehlerstand)
	else:		
		print ("Fehler:"+Fehler)
...
try:
	...
	if (response.status_code == 200):
		...
		print ("Fehlermeldung vom Heisopo"+ response.status_code)
